...
const getChores = async (req, res) => {
    try {
        ...
        const chores = await client.query(`SELECT * FROM chores WHERE person = '${person}';`);
...
const createChore = async (req, res) => {
    try {
        ...
        await client.query(`INSERT INTO chores (_id, name, index, date, timer, person) VALUES ('${_id}', '${name}', '${index}', '${date}', '${timer}', '${person._id}');`);
...
const updateName = async (req, res) => {
    try {
        ...
        const { person } = req.body;
        await client.query(`UPDATE chores SET name = '${name}' WHERE _id = '${_id}' AND person = '${person._id}';`);
        res.status(200).json({
...
const updateIndex = async (req, res) => {
    try {
        ...
        const { person } = req.body;
        await client.query(`UPDATE chores SET index = '${index}' WHERE _id = '${_id}' AND person = '${person._id}';`);
        res.status(200).json({
...
const updateDate = async (req, res) => {
    try {
        ...
        const { person } = req.body;
        await client.query(`UPDATE chores SET date = '${date}' WHERE _id = '${_id}' AND person = '${person._id}';`);
        res.status(200).json({
...
const updateTimer = async (req, res) => {
    try {
        ...
        const { person } = req.body;
        await client.query(`UPDATE chores SET timer = '${timer}' WHERE _id = '${_id}' AND person = '${person._id}';`);
        res.status(200).json({
...
const deleteChore = async (req, res) => {
    try {
        ...
        const { person } = req.body;
        await client.query(`DELETE FROM chores WHERE _id = '${_id}' AND person = '${person._id}';`);
...
export { getChores, createChore, updateName, updateIndex, updateDate, updateTimer, deleteChore, };
